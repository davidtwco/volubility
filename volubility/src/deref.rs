...
impl<S: Deref> DerefNode for Resource<S> {
    ...
    fn deref_node<'a>(&'a self) -> Self::Target<'a> {
        Resource { body: self.body.iter().map(|entry| entry.deref_node()).collect() }
    }
}
...
impl<S: Deref> DerefNode for Entry<S> {
    ...
    fn deref_node<'a>(&'a self) -> Self::Target<'a> {
        match self {
            ...
            Entry::Junk { content } => Entry::Junk { content: content.deref() },
        }
    }
}
...
impl<S: Deref> DerefNode for Comment<S> {
    ...
    fn deref_node<'a>(&'a self) -> Self::Target<'a> {
        Comment { content: self.content.iter().map(|line| line.deref()).collect() }
    }
}
...
impl<S: Deref> DerefNode for Identifier<S> {
    ...
    fn deref_node<'a>(&'a self) -> Self::Target<'a> {
        Identifier { name: self.name.deref() }
    }
}
...
impl<S: Deref> DerefNode for Message<S> {
    ...
    fn deref_node<'a>(&'a self) -> Self::Target<'a> {
        Message {
            ...
            attributes: self.attributes.iter().map(|attr| attr.deref_node()).collect(),
...
impl<S: Deref> DerefNode for Attribute<S> {
    ...
    fn deref_node<'a>(&'a self) -> Self::Target<'a> {
        Attribute { id: self.id.deref_node(), value: self.value.deref_node() }
    }
}
...
impl<S: Deref> DerefNode for Pattern<S> {
    ...
    fn deref_node<'a>(&'a self) -> Self::Target<'a> {
        Pattern { elements: self.elements.iter().map(|el| el.deref_node()).collect() }
    }
}
    ...
    fn deref_node<'a>(&'a self) -> Self::Target<'a> {
        match self {
            PatternElement::TextElement { value } => {
                PatternElement::TextElement { value: value.deref() }
            }
            PatternElement::Placeable { expression } => {
                PatternElement::Placeable { expression: expression.deref_node() }
            }
        }
    }
}
    ...
    fn deref_node<'a>(&'a self) -> Self::Target<'a> {
        match self {
            Expression::Select { selector, variants } => Expression::Select {
                ...
                variants: variants.iter().map(|variant| variant.deref_node()).collect(),
            },
    ...
    fn deref_node<'a>(&'a self) -> Self::Target<'a> {
        match self {
            ...
            VariantKey::NumberLiteral { value } => {
                VariantKey::NumberLiteral { value: value.deref() }
            }
        }
    }
}
    ...
    fn deref_node<'a>(&'a self) -> Self::Target<'a> {
        match self {
            InlineExpression::StringLiteral { value } => {
                InlineExpression::StringLiteral { value: value.deref() }
            }
            InlineExpression::NumberLiteral { value } => {
                InlineExpression::NumberLiteral { value: value.deref() }
            }
            ...
            InlineExpression::VariableReference { id } => {
                InlineExpression::VariableReference { id: id.deref_node() }
            }
            InlineExpression::Placeable { expression } => {
                InlineExpression::Placeable { expression: Box::new(expression.deref_node()) }
            }
        }
    }
}
...
impl<S: Deref> DerefNode for NamedArgument<S> {
    ...
    fn deref_node<'a>(&'a self) -> Self::Target<'a> {
        NamedArgument { name: self.name.deref_node(), value: self.value.deref_node() }
    }
}
